...
export const BUILD_ALL_STEPS = [
  ...
  { label: "build-stamp", kind: "node", args: ["scripts/build-stamp.mjs"] },
  {
    ...
    // fork: non-fatal — DTS errors from upstream merge drift don't block the JS publish
    optional: true,
  },
  {
...
if (isMainModule()) {
  for (const step of BUILD_ALL_STEPS) {
    ...
    if (typeof result.status === "number") {
      if (result.status !== 0) {
        if (step.optional) {
          console.error(`[build-all] ${step.label} failed (optional, continuing)`);
          continue;
        }
    ...
    if (step.optional) {
      console.error(`[build-all] ${step.label} did not start (optional, continuing)`);
      continue;
    }
    process.exit(1);
  }
