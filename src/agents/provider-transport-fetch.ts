...
export function buildGuardedModelFetch(model: Model<Api>): typeof fetch {
  ...
  return async (input, init) => {
    ...
    const result = await fetchWithSsrFGuard({
      ...
      // When using env-proxy, skip DNS pinning: the proxy handles routing and
      // fake-ip (Clash/V2Ray TUN mode) causes legitimate hostnames to resolve
      // to private/reserved IPs on the client side, triggering false SSRF blocks.
      ...(dispatcherPolicy?.mode === "env-proxy"
        ? { proxy: "env" as const, dangerouslyAllowEnvProxyWithoutPinnedDns: true }
        : {}),
    });
